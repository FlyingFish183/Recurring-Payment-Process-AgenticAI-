...
export function parseInvoiceFromText(raw: string): Record<string, unknown> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      if (d) {
        ...
        let y = Number(d[3]);
        if (y < 100) y += 2000;
        // Prefer M/D/Y when first > 12 is impossible as month; else keep as-is (US-style for demo invoices)
  ...
  // Seller / bill-from: line before "Bill To" or after "From"
  ...
  const acctMatch = raw.match(
    /(?:account\s*(?:no|number|#)|stk|số\s*tài\s*khoản)\s*[:.]?\s*([\d\s-]{8,22})/i,
  );
  if (acctMatch) out.accountNumber = acctMatch[1].replace(/\D/g, "");

  const bankMatch = raw.match(/(?:bank|ngân\s*hàng)\s*[:.]?\s*([A-Za-z][A-Za-z0-9 .&-]{2,40})/i);
